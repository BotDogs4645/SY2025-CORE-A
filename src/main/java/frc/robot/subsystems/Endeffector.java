...
public class EndEffector extends SubsystemBase {
  ...
  public EndEffector() {
    this.mainMotor = new TalonFX(17);
    this.pivotMotor = new TalonFX(18);
    this.input = new DigitalInput(1);
    this.pivotEncoder = new DutyCycleEncoder(input);
    ...
    talonConfig = new TalonFXConfiguration()
        .withFeedback(
            new FeedbackConfigs()
                .withSensorToMechanismRatio(Constants.EndEffectorConstants.pivotGearRatio));
  ...
  public void alignToIntake() {
    MotionMagicVoltage intake = new MotionMagicVoltage(Constants.EndEffectorConstants.intake.getRotations());
    pivotMotor.setControl(intake);
  }

  public void setPivotLevel(int level) {
    switch (level) {
      case 1:
